...
@section('header_title', 'Add Employee')

@section('content')
<div class="card">
    <h3 style="margin-top:0;">Add New Employee / Position</h3>
    ...
    <form action="{{ route('admin.profile.structure.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group" style="margin-bottom: 15px;">
                ...
                <input type="text" name="name" required style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px;" value="{{ old('name') }}">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group" style="margin-bottom: 15px;">
                ...
                <input type="text" name="position" required placeholder="Example: Head of Department" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px;" value="{{ old('position') }}">
            ...
            <div class="form-group" style="margin-bottom: 15px;">
                ...
                <select name="parent_id" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px;">
                    ...
                    @foreach($parents as $parent)
                        <option value="{{ $parent->id }}" {{ old('parent_id') == $parent->id ? 'selected' : '' }}>{{ $parent->position }} - {{ $parent->name }}</option>
                    @endforeach
                </select>
